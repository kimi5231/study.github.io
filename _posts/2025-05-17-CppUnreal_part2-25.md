...
* 정점을 늘리지 않고 픽셀 단위로 노멀 벡터의 값을 저장해 굴곡을 입히는 방법
* 텍스쳐 매핑처럼 uv좌표에 각각 해당하는 노멀 벡터의 정보를 입력하여 굴곡을 표현
* 모델 좌표계에서도 모델은 계속 변해 노멀 벡터의 값을 고정할 수 없어, 별도의 좌표계인 Tangent Space를 사용
* Tangent Space: 노멀 벡터를 직축으로 하는 공간, 픽셀마다 각자의 Tangent Space를 가지고 있음
* 쉐이더 코드에서는 카메라 좌표를 기준으로 연산을 진행하기 때문에 Tangent Space 좌표를 카메라 좌표로 바꿔 사용해야 함
{% endcapture %}
